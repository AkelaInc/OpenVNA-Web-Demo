...
class VnaThread():
	def __init__(self, vna_no, command_queue, response_queue, exit_value):
		...
		self.start_f     = config.VNA_STARTF
		self.stop_f      = config.VNA_STOPF
		self.npts_s      = config.VNA_NPTS
	...
	def tryLoadLocalCal(self):
		...
		if not os.path.exists(fname):
			self.log.warn("Could not find local cal file '%s'.", fname)
			return
...
def threadProcess(vna_no, command_queue, response_queue, exit_value):
	vnat = VnaThread(vna_no, command_queue, response_queue, exit_value)
	# Ignore ctrl+c events
	signal.signal(signal.SIGINT, signal.SIG_IGN)

	vnat.run()
	print("VNA Thread exited")

def create_thread(vna_no, command_queue, response_queue, exit_value):
	proc = multiprocessing.Process(target=threadProcess, args=(vna_no, command_queue, response_queue, exit_value))
	proc.start()
	return proc
